...
def create_dataset(cloud_size):
    # Create samples of overlapping point clouds from dublin
    ...
    for fidx, fi in enumerate(p.imap_unordered(load_laz, laz_files[1:])):
        ...
        queries = kdtree._query(kd, f1_sample[:,:3], k=cloud_size, dmax=1)
        for idx, query in enumerate(queries):
            if len(query) == cloud_size:
